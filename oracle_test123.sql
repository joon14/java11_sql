...
-- 생일(birth)이 1990년생~1996년생인 회원의 id, name 컬럼 검색
select id, name from member where birth between '1990-01-01' and '1996-12-31';
...
-- id가 'kim', 'lee', 'cho', 'kang' 인 회원(member) 정보를 검색
select * from member where id='kim' or id='lee' or id='cho' or id='kang';
...
-- 뷰 제거
drop view view_emp;
-- 뷰 수정 : create or replace view view_emp2 as select * from where no>=2 or name like '%이%';
...
-- 집합연산 시 연산하는 두 개의 테이블의 구조가 같거나 연산하는 컬럼타입이 같아야 함.
