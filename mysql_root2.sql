...
select address from member where id='lsh';
...
-- 참조하는 테이블이 존재해야 하므로 임의의 테이블 bookkind(kindcode, kindname) 생성
create table bookkind(kindcode varchar(3) primary key, kindname varchar(50));
...
select distinct * from member where id in (select id from sales); 
...
select count(*) as "count" from sales where amount>=2;

-- 30) 도서(book) 테이블에서 
-- 단가(bookprice)가 19000이상 30000이하인 데이터의 도서명(booktitle), 도서가격(bookprice), 저자(author) 를 조회하시오.
select booktitle, bookprice, author from book where bookprice between 19000 and 30000;

-- 31) 도서(book) 테이블에서 출판사(pubcom)이 한빛미디어 이거나 
-- 남가람북스인 튜플의 도서명(booktitle), 저자(author), 수량(bookcount)를 조회하시오.
...
-- 34) 도서(book) 테이블에서 출판일(pubdate)을 기준으로 오름차순하여 검색하되 
-- 출판일(pubdate)이 같은 경우 도서코드(bookid)의 내림차순으로 하시오.
...
-- alias는 영문이 아닌 경우나 띄어쓰기가 있는 경우에는 따옴표("")를 사용해야한다.
...
-- 42) 판매(sales) 테이블에서 
-- 판매금액(money)가 큰 순으로 3위 까지인 튜플의 판매코드(sno), 도서코드(bno), 회원아이디(id)가 출력될 수 있도록 하시오.
select sno, bno, id from sales order by money desc limit 3;

-- 43) 회원 뷰(mem_view)를 생성하되 
-- 회원2(member2) 테이블을 활용하고, 회원 데이터 중에서 가입일을 기준으로 2022년 09월 이후에 가입한 회원을 대상으로 하시오.
...
-- create sequence sd_seq start with1 increment by 1;
...
-- 58) 내부조인을 활용하여 구매한 적이 있는 회원아이디(id), 이름(name), 연락처(tel) 을 조회하시오.
select a.id, a.name, a.tel from member a inner join sales b on a.id=b.id;

-- 59) 내부조인을 활용하여 구매한 도서의 도서코드(bookid), 도서명(booktitle), 총판매수량합계(amount), 총판매금액(money)을 조회하시오.
-- 단, 출력내용은 도서코드(bookid)의 오름차순으로 하시오.
select a.bookid, a.booktitle, sum(b.amount) as "총판매수량합계", sum(b.money) as "총판매금액" from book a inner join sales b on a.bookid=b.bno
group by a.bookid, a.booktitle order by a.bookid; 
...
select a.id, a.name from member a left join sales b on a.id=b.id where b.id is null;
...
-- 서브쿼리를 이용한 차집합 뷰 작성
create view minus_book_view as (select bookid, bookkind, booktitle, bookcount from book 
where bookid not in(select distinct bookid from book2));
select * from minus_book_view2;
-- 외부조인을 이용한 차집합 뷰 작성
create view minus_book_view2 as (select a.bookid, a.bookkind, a.booktitle, a.bookcount from book a
left join book2 b on a.bookid=b.bookid where b.bookid is null);
select * from minus_book_view2;

create view minus_book_view3 as (select a.bookid, a.bookkind, a.booktitle, a.bookcount from book a 
where not exists (select 1 from book2 b where a.bookid=b.bookid));
select * from minus_book_view3;
